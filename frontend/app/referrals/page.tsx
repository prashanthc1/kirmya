...
export default function ReferralsPage() {
  ...
  const fetchReferrals = async () => {
    try {
      const data = await api.get<{ referrals: Referral[] }>(
        "/referrals/outgoing",
      );
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Track the referral requests you&apos;ve sent and where each one
              stands.
            </p>
          </div>
          ...
          {showForm && (
            ...
            >
              <form
                ...
                className="bg-card border border-border/80 p-6 rounded-3xl space-y-4 shadow-sm"
              >
          ...
          {loading ? (
            ...
            <div className="text-center py-16 border border-dashed border-border/80 rounded-3xl p-8 bg-secondary/15 space-y-3">
              ...
              <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                Request connections at hiring companies to skip the screening
                queue. You can send a request from the button above.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {referrals.map((ref) => {
                ...
                return (
                  <div
                    key={ref.id}
                    className="bg-card border border-border/60 p-5 rounded-2xl flex flex-col gap-3 hover:border-border transition-all"
                  >
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold border flex items-center gap-1 shrink-0 ${conf.bg}`}
                      >

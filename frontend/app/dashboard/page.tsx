...
export default function DashboardPage() {
  ...
  const [coachReplies, setCoachReplies] = useState<Array<{ sender: "user" | "coach"; text: string }>>([
    { sender: "coach", text: "Hello! I noticed you have some active applications. How can I help you prepare today?" }
  ]);
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <div className="flex-grow flex flex-col items-center justify-center py-20 gap-3">
          ...
          <span className="text-sm font-semibold text-muted-foreground">Analyzing career metrics...</span>
        </div>
        <SiteFooter />
      </div>
    );
      ...
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <main className="flex-grow max-w-lg mx-auto w-full px-4 py-20">
          <div className="p-6 bg-destructive/10 border border-destructive/20 rounded-3xl text-destructive text-center space-y-3">
            <p className="text-sm font-bold">{error || "Could not load dashboard."}</p>
            <button onClick={loadData} className="px-4 py-2 bg-destructive text-destructive-foreground rounded-full text-xs font-bold">
              Try Again
            </button>
          </div>
        </main>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="space-y-1">
          <span className="text-xs font-bold uppercase tracking-widest text-primary">Workspace Overview</span>
          <h1 className="text-3xl font-extrabold tracking-tight">Good morning, {firstName}.</h1>
          <p className="text-sm text-muted-foreground">Here&apos;s a quick snapshot of your professional recovery network activity.</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Active Applications", value: summary?.job_seeker.applications ?? 0, icon: Briefcase, color: "text-blue-500" },
            { label: "Saved Openings", value: summary?.job_seeker.saved_jobs ?? 0, icon: Bookmark, color: "text-indigo-500" },
            { label: "Connections", value: connections.length, icon: Users, color: "text-emerald-500" },
            { label: "Profile Completeness", value: `${profile.profile_completeness_score}%`, icon: CheckCircle, color: "text-amber-500" }
          ].map((stat, idx) => {
            const Icon = stat.icon;
            return (
              <div key={idx} className="bg-card border border-border/60 p-5 rounded-2xl space-y-3 shadow-sm hover:border-border transition-all">
            ...
            <div className="bg-card border border-border/80 p-6 rounded-3xl shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
                ...
                <div className="space-y-1 text-center sm:text-left flex-grow">
                  ...
                  <p className="text-sm font-semibold text-muted-foreground">{profile.headline || "Add a career headline"}</p>
                  <div className="flex items-center justify-center sm:justify-start gap-1 text-xs text-muted-foreground">
                    <MapPin className="h-3.5 w-3.5" />
                    <span>{profile.location || "Add location"} &bull; {profile.open_to_remote ? "Open to Remote" : "On-site"}</span>
                  </div>
                </div>

                <div className="flex gap-2 shrink-0">
                  <Link href="/profile/edit" className="px-4 py-2 rounded-full border border-border hover:bg-secondary text-xs font-bold flex items-center gap-1">
                    <Edit className="h-3.5 w-3.5" />
                    Edit Profile
                  </Link>
                  <Link href="/profile" className="px-4 py-2 rounded-full bg-primary hover:bg-primary/95 text-primary-foreground text-xs font-bold flex items-center gap-1 shadow-sm">
                  ...
                  {incomingRequests.map((req) => (
                    <div key={req.id} className="p-4 bg-secondary/15 border border-border/40 rounded-2xl flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link href={`/profile/${req.requester_id}`} className="text-sm font-bold hover:underline">
                            {req.requester_name}
                          </Link>
                          <p className="text-xs text-muted-foreground line-clamp-1">{req.requester_headline || "Professional"}</p>
                        </div>
                      </div>
            ...
            <div className="bg-card border border-border/80 p-6 rounded-3xl shadow-sm space-y-4">
              ...
              {connections.length === 0 ? (
                <div className="text-center py-8 border border-dashed border-border/60 rounded-2xl p-6 bg-secondary/15 space-y-3">
                  <p className="text-xs text-muted-foreground">Get connected with peers, sponsors, and mentors to unlock messaging.</p>
                  <Link href="/search?type=user" className="inline-flex px-4 py-2 rounded-full border border-border hover:bg-secondary text-xs font-bold">
                    Find People
                  </Link>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {connections.map((c) => {
                    ...
                    const connHeadline = isReq ? c.receiver_headline : c.requester_headline;
                    const connID = isReq ? c.receiver_id : c.requester_id;
                    return (
                      <div key={c.id} className="p-4 bg-secondary/15 border border-border/40 rounded-2xl flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <Link href={`/profile/${connID}`} className="text-xs font-bold hover:underline block truncate">
                              {connName}
                            </Link>
                            <span className="text-[10px] text-muted-foreground block truncate">{connHeadline || "Professional"}</span>
                          </div>
                        </div>
                        <Link href="/inbox" className="p-1.5 rounded-full hover:bg-secondary text-muted-foreground hover:text-foreground transition-all">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border border-border/80 rounded-3xl shadow-sm p-6 space-y-4">
              ...
              <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                {coachReplies.map((reply, idx) => (
                  <div key={idx} className={`p-3 rounded-2xl text-xs leading-relaxed max-w-[90%] ${
                    reply.sender === "coach"
            ...
            <div className="bg-card border border-border/80 p-6 rounded-3xl shadow-sm space-y-4">
              ...
              <div className="space-y-4">
                {[
                  { title: "VP, Supply Chain Operations", company: "Atlas Co", location: "Bangalore / Remote" },
                  { title: "Director of Logistics & Operations", company: "Vertex Corp", location: "Mumbai / Hybrid" }
                ].map((rec, idx) => (
                  <div key={idx} className="space-y-1 block group">
                    <Link href="/jobs" className="text-xs font-bold group-hover:text-primary transition-colors block">
                      {rec.title}
                    </Link>
              ...
              <Link href="/jobs" className="text-xs font-bold text-primary flex items-center gap-1 hover:underline pt-2 border-t border-border/40">
                Explore all jobs

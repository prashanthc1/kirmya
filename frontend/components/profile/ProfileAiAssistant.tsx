...
import { 
  Sparkles, Send, Loader2, RefreshCw, Check, ArrowRight,
  TrendingUp, DollarSign, BrainCircuit, FileText, CheckCircle2
} from "lucide-react";
...
interface ProfileAiAssistantProps {
  ...
  onApplyAiChange: (sectionId: string, updatedFields: Partial<ExtendedProfile>) => void;
}
...
export default function ProfileAiAssistant({
  ...
  const [messages, setMessages] = useState<Array<{ role: "user" | "assistant"; content: string; actions?: { type: string; label: string; data: Partial<ExtendedProfile> } | null }>>([
    {
      role: "assistant",
...
"Supply chain architect with 22 years of operations leadership. I specialize in turning complex logistics networks from cost-centers into margin-drivers through linear programming and automation. I've saved $6.4M in annual overhead and maintained 99.2% on-time delivery. Let me build your next-generation fulfillment pipeline."`);
  ...
  const handleSend = (textToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      if (finalQuery.toLowerCase().includes("salary") || finalQuery.toLowerCase().includes("market")) {
...
**High Demand Keywords**: *SAP EWM, S&OP Optimization, Contract Negotiation, Warehouse Robotics*. You have 3/4. Let's add *Warehouse Robotics* to your skills to boost search visibility.`;
      } else if (finalQuery.toLowerCase().includes("star") || finalQuery.toLowerCase().includes("rewrite")) {
...
*"Spearheaded supply chain network optimization that consolidated 14 distribution facilities into 8 regional hubs, reducing overall cost-per-unit by 18% and saving $6.4M in annual lease and labor overhead."*`;
        actions = {
          ...
          data: {
            experiences: profile.experiences.map(e => 
              e.id === "exp_1" 
                ? { ...e, achievements: ["Spearheaded supply chain network optimization that consolidated 14 distribution facilities into 8 regional hubs, reducing overall cost-per-unit by 18% and saving $6.4M in annual lease and labor overhead.", ...e.achievements.slice(1)] }
                : e
            )
          }
        };
  ...
  const handleApplyAction = (action: { type: string; label: string; data: Partial<ExtendedProfile> }) => {
    if (action.type === "experience_rewrite") {
      ...
      setMessages(prev => [...prev, { 
        role: "assistant", 
        content: "✅ Successfully applied the STAR bullet optimization to your Cascade Freight experience!" 
      }]);
    }
  };
  ...
  const applyContextSuggestion = () => {
    if (activeSectionId === "experience") {
      ...
      const updated = {
        experiences: profile.experiences.map(e => 
          e.id === "exp_1" 
            ? { ...e, achievements: ["Spearheaded mathematical location-placement algorithms to merge 14 facilities into 8 hubs, reducing overall cost-per-unit by 18% and saving $6.4M in annual lease and labor overhead."] }
            : e
        )
      };
      ...
      onApplyAiChange("summary", {
        elevator_pitch: "Supply chain architect with 22 years of operations leadership. I specialize in turning complex logistics networks from cost-centers into margin-drivers through linear programming and automation. I've saved $6.4M in annual overhead and maintained 99.2% on-time delivery. Let me build your next-generation fulfillment pipeline."
      });
      setAppliedText(true);
    }
  };
      ...
      <div className="bg-card border border-border/80 rounded-3xl p-6 shadow-sm flex flex-col h-[650px]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-bold text-foreground">Kirmya Career Assistant</h3>
        ...
        <div className="flex border-b border-border/40 py-2.5 shrink-0 gap-1">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as "chat" | "insights" | "optimize")}
        ...
        <div className="flex-grow overflow-y-auto py-4 space-y-4 pr-1">
                ...
                {isGenerating && (
                  <div className="flex justify-start">
                    <div className="bg-secondary/60 border border-border/40 p-3 rounded-2xl rounded-tl-none flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-semibold text-muted-foreground">Drafting recommendations...</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4 text-xs">
              <div className="bg-primary/5 border border-primary/10 p-4 rounded-2xl space-y-2">
                ...
                <p className="text-muted-foreground leading-normal">
                  Select any section card on the center workspace to run automatic optimizations.
                </p>
                {activeSectionId ? (
                  <div className="bg-card border border-border/60 p-2.5 rounded-xl text-[10px] font-bold text-foreground">
                    ⚡ Currently inspecting: <span className="text-primary capitalize">{activeSectionId}</span>
                  </div>
                ) : (
              ...
              {aiSuggestions ? (
                <div className="border border-border/80 p-4 rounded-2xl space-y-3 bg-secondary/35">
                  ...
                  <p className="text-muted-foreground whitespace-pre-wrap leading-relaxed">{aiSuggestions}</p>
                ...
                <div className="text-center py-8 text-muted-foreground">
                  <p>Choose &quot;Professional Summary&quot; or &quot;Work Experience&quot; in the main center panel to generate quick copy.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "insights" && (
            <div className="space-y-4 text-xs">
              ...
              <div className="bg-card border border-border/80 p-4 rounded-2xl space-y-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 text-center">
                  <div className="bg-secondary/40 border border-border/40 p-2 rounded-xl">
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Median Comp</p>
                    <p className="text-sm font-black text-foreground mt-0.5">$215,000</p>
                  </div>
                  <div className="bg-secondary/40 border border-border/40 p-2 rounded-xl">
                    <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">Demand Level</p>
                    <p className="text-sm font-black text-emerald-500 mt-0.5">High (+12%)</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-card border border-border/80 p-4 rounded-2xl space-y-3 shadow-sm">
                <h5 className="font-bold text-foreground">Top Recruiter Search Keywords</h5>
                <div className="flex flex-wrap gap-1.5">
                  {["S&OP Modeling", "Warehouse Layouts", "Contract Negotiation", "WMS Architect", "Linear Programming", "OSHA Compliance"].map(kw => (
                    <span key={kw} className="px-2 py-1 bg-secondary text-muted-foreground rounded-md text-[10px] font-semibold border border-border/40">
                      {kw}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border border-blue-500/20 p-4 rounded-2xl space-y-2">
                ...
                <p className="text-muted-foreground text-[11px] leading-relaxed">
                  Run a custom market analytics report comparing your qualifications against open requisitions at major logistics and e-commerce companies.
                </p>
                <button
                  onClick={() => handleSend("Analyze my target market salary and recruiter demand")}

...
import { 
  User, CheckCircle, Award, TrendingUp, ShieldCheck, 
  FileCheck, Download, ExternalLink, HelpCircle, 
  RotateCcw, RotateCw, RefreshCw, UploadCloud, AlertCircle
} from "lucide-react";
    ...
    <aside className="w-full lg:w-80 flex-shrink-0 lg:sticky lg:top-24 space-y-6 self-start">
        ...
        <div className="relative pt-10 flex flex-col items-center text-center">
          ...
          <div className="relative group cursor-pointer" onClick={handleAvatarClick}>
            ...
            <div className="absolute inset-2.5 rounded-full overflow-hidden bg-secondary border border-border flex items-center justify-center">
              {profile.photo_url ? (
                <img src={profile.photo_url} alt={profile.headline} className="w-full h-full object-cover" />
              ) : (
                <span className="text-xl font-bold text-primary">
                  {profile.preferred_name?.charAt(0) || profile.headline?.charAt(0) || "K"}
                </span>
              )}
            </div>
      ...
      <div className="bg-card border border-border/80 rounded-3xl p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {[
            { label: "Career Readiness", val: atsScore + 2, icon: Award, color: "text-blue-500 bg-blue-500/5 border-blue-500/20" },
            { label: "ATS Score", val: atsScore, icon: FileCheck, color: "text-emerald-500 bg-emerald-500/5 border-emerald-500/20" },
            { label: "Recruiter Reach", val: recruiterScore, icon: TrendingUp, color: "text-violet-500 bg-violet-500/5 border-violet-500/20" },
            { label: "Verification Score", val: verificationScore, icon: ShieldCheck, color: "text-indigo-500 bg-indigo-500/5 border-indigo-500/20" }
          ].map((score) => {
            const Icon = score.icon;
            return (
              <div key={score.label} className={`border p-3 rounded-2xl flex flex-col justify-between gap-2.5 ${score.color}`}>
                <div className="flex justify-between items-start">
                  <Icon className="h-4 w-4 opacity-80" />
                  <span className="font-black text-base leading-none">{score.val}%</span>
                </div>
                <span className="text-[10px] font-bold leading-tight uppercase tracking-wider text-muted-foreground/80">{score.label}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="bg-card border border-border/80 rounded-3xl p-5 shadow-sm space-y-3">
        <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Quick Actions</h4>
      ...
      <div className="bg-card border border-border/80 rounded-3xl p-5 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">AI Profile Booster</h4>
        ...
        <div className="space-y-2.5">
          {[
            { label: "Connect calendar link for booking", score: "+5%", section: "identity" },
            { label: "Add Elevator Pitch to branding", score: "+8%", section: "summary" },
            { label: "Add KPIs to Northstar work experience", score: "+10%", section: "experience" },
            { label: "Complete Skills verification test", score: "+12%", section: "verification" }
          ].map((item, idx) => (
            <button
              key={idx}
            ...
            >
              <div className="space-y-0.5">
                <p className="font-semibold text-foreground leading-snug group-hover:text-primary transition-colors">{item.label}</p>
                <p className="text-[10px] text-muted-foreground">Scrolls to section</p>
              </div>
              <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/5 px-1.5 py-0.5 rounded shrink-0">{item.score}</span>
            </button>
          ))}
        </div>
      </div>
